    ...
    <div class="flex flex-col gap-9">
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h3 class="font-medium text-black dark:text-white">Life Goal Form</h3>
            </div>
            <form action="{{ route('web.app.life.goals.store') }}" method="POST">
                @csrf
                <div class="p-6.5">
                    <div class="mb-4.5">
                        ...
                        <input type="text" name="goal_name" placeholder="Enter your goal name" 
                               ...
                               value="{{ old('goal_name') }}" required />
                        @if ($errors->has('goal_name'))
                            <span class="text-danger text-sm">{{ $errors->first('goal_name') }}</span>
                        @endif
                    </div>
                    ...
                    <div class="mb-4.5">
                        ...
                        <input type="text" id="current_amount" placeholder="Enter current amount" 
                               ...
                               name="current_amount" value="{{ old('current_amount', 0) }}" />
                        @if ($errors->has('current_amount'))
                            <span class="text-danger text-sm">{{ $errors->first('current_amount') }}</span>
                        @endif
                    </div>
                    ...
                    <div class="mb-4.5">
                        ...
                        @if ($errors->has('target_amount'))
                            <span class="text-danger text-sm">{{ $errors->first('target_amount') }}</span>
                        @endif
                    </div>
                    ...
                    <div class="mb-4.5">
                        ...
                        @if ($errors->has('deadline'))
                            <span class="text-danger text-sm">{{ $errors->first('deadline') }}</span>
                        @endif
                    </div>
